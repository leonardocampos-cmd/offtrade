...
def main():
    ...
    try:
        ...
        step("9e - Contagem de estoque (contagem_estoque_data.js)")
        try:
            import subprocess, sys as _sys
            result = subprocess.run(
                [_sys.executable, "exportacao_contagem_estoque.py"],
                capture_output=True, text=True, timeout=600
            )
            print(result.stdout)
            if result.returncode != 0:
                print("[AVISO] exportacao_contagem_estoque falhou — ignorado, pipeline continua.")
                print(result.stderr)
        except Exception:
            print("[AVISO] exportacao_contagem_estoque falhou — ignorado, pipeline continua.")
            traceback.print_exc()

        step("9f - Vendas Perini vs. preço estipulado (perini_vendas_data.js)")
        try:
            import subprocess, sys as _sys
            result = subprocess.run(
                [_sys.executable, "exportacao_perini_vendas.py"],
                capture_output=True, text=True, timeout=600
            )
            print(result.stdout)
            if result.returncode != 0:
                print("[AVISO] exportacao_perini_vendas falhou — ignorado, pipeline continua.")
                print(result.stderr)
        except Exception:
            print("[AVISO] exportacao_perini_vendas falhou — ignorado, pipeline continua.")
            traceback.print_exc()
