...
LEFT JOIN (
    ...
    WHERE CODOPER = 'S' AND NUMNOTADEV IS NULL AND DTCANCEL IS NULL
      AND CODFILIAL IN (1, 2)
    GROUP BY CODCLI
) LV ON C.CODCLI = LV.CODCLI
LEFT JOIN MGON.PCMOV M
...
LEFT JOIN MGON.PCPRODUT P  ON M.CODPROD   = P.CODPROD
LEFT JOIN MGON.PCFORNEC F  ON P.CODFORNEC = F.CODFORNEC
LEFT JOIN MGON.PCUSUARI U  ON C.RCA       = U.CODUSUR
WHERE U.NOME LIKE '%OFF TRADE%'
  AND C.CODCLI NOT IN (
      SELECT DISTINCT CODCLI FROM MGON.PCMOV
...
print(f"OK {df['CODCLI'].nunique()} clientes nao positivados MG")
...
out_path = Path(__file__).parent / "nao_pos_mg_data.js"
out_path.write_text(
    f"// Gerado automaticamente pelo exportacao_nao_pos_mg.py\n\n"
    f"const NAO_POS_MG_DATA = {json.dumps(payload, ensure_ascii=False, indent=2)};\n",
    encoding='utf-8'
)
