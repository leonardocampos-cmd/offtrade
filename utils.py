...
# Só essa pessoa vê "app" (home) e "Admin Objetivos" na barra lateral do
# Streamlit — os outros admins de EMAILS_ADMIN continuam com acesso à página
# em si (Admin_Objetivos.py checa rca["admin"] por conta própria), só não
# aparece no menu pra não confundir vendedor/RCA comum que só usa Crédito e
# Cadastro (pedido em 2026-08-04).
SUPER_ADMIN_EMAIL = "leonardo.campos@rigarr.com.br"
...
def back_button(url: str = "https://offtrade.duckdns.org/"):
    """Link 'voltar' pro hub estático — necessário desde que 'app' (home) saiu
    do menu lateral pra quem não é SUPER_ADMIN_EMAIL, então quem chega direto
    numa página (ex: vendedor clicando 'Pedir limite e cadastro' em
    metas.html, que abre em nova aba) não tinha mais como sair do Streamlit
    sem fechar a aba.

    Usa <a> comum (mesma aba) em vez de st.link_button: esse sempre abre em
    nova aba, e a nova aba perde o sessionStorage que auth.js (site estático)
    usa pra liberar acesso — sem ele, index.html manda direto pra login.html
    mesmo com a pessoa já autenticada (confirmado em 2026-08-04).

    target="_self" é obrigatório aqui: o st.markdown do Streamlit injeta
    ...
    st.markdown(
        ...
        f'font-size:.85rem;font-weight:700">← Voltar</a>',
